...
var Db = function(){
    ...
    _self.getArticleTitle = function(repostTask, cb){
        ...
        dbWeibo.query(sql, [repostTask.micro_blog_id], function(err, results){
            if(err || results.length != 1){
                repostTask.title = '';
                callback(null, repostTask);
                return;
            }
            var table = 'bulletin';
            if(results[0].content_type == 'zixun'){
                table = 'article_content';
            }
            var sql = "SELECT title FROM " + table + " WHERE id = ?";
            dbSpider.query(sql, [results[0].article_id], function(err, results){
                if(err || results.length != 1){
                    repostTask.title = '';
                    cb(null, repostTask);
                    return;
                }else{
                    repostTask.title = results[0].title;
                    cb(null, repostTask);
                }
            });
        });
    ...
    _self.getRepostTask = function(uri, cb){
        ...
        async.waterfall([start, _self.getArticleTitle, _self.getSentMicroBlog, end], function(err){
